    ...
    return (
      <g transform={transform}>
        ...
        <text is visibility={visibility} transform={transformText}>
          <tspan is x="0" class={this.props.textStyle1}
                 text-anchor="middle">{this.props.xValue + " : " + this.props.tooltip.data.key}</tspan>
          <tspan is x="0" class={this.props.textStyle2} text-anchor="middle"
                 dy="25">{this.props.yValue + " : " + this.props.tooltip.data.value}</tspan>
        </text>
      </g>
    );
  }
...
var D3Dots = React.createClass({
  ...
  render: function () {
    ...
    var circles = data.map(function (d, i) {
      ...
      return (<circle className="dot" r={_self.props.r} cx={_self.props.x(d[_self.props.xData])}
...
var D3TimeLineChart = React.createClass({
  ...
  createElements: function (element, i) {
    ...
    switch (element.type) {
      ...
      case 'xGrid':
        object =
          <D3Grid h={this.h} len={this.h} scale={this.xScale} gridType="x" key={i} {...this.props} {...element.props}/>;
        break;

      case 'yGrid':
        object =
          <D3Grid h={this.h} len={this.w} scale={this.yScale} gridType="y" key={i} {...this.props} {...element.props}/>;
        break;
      ...
      case 'area':
        object =
          <path className={element.props.className} d={this.area(this.props.data)} key={i} fill={element.props.fill}/>;
        break;
      case 'path':
        object = <path className={element.props.className} d={this.line(this.props.data)}
                       strokeLinecap={element.props.strokeLinecap} key={i}/>;
        break;
        ...
        <div className="col-md-12 custom_padding">
          <Panel>
            <D3TimeLineChart data={data} xData="date" yData="count" margin={margin}
              ...
              <tooltip textStyle1="tooltip-text1" textStyle2="tooltip-text1" bgStyle="tooltip-bg" xValue="Date"
                       yValue="Count"/>
            </D3TimeLineChart>
          </Panel>
        </div>
...
class UsersOverTime extends React.Component {
  render () {
    return (
      <div>
        <h2 styleName="sectionTitle">Users Over Time</h2>
        <div styleName="usersOverTime">
          <MainContainer />
        </div>
      </div>
    );
...
export default CSSModules(UsersOverTime, styles);
