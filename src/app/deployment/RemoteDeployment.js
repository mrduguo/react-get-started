...
class RemoteDeployment extends React.Component {
    ...
    handleNext = () => {
        ...
        if (stepIndex == 0 && store.getState().form.config.values) {
        ...
        if (stepIndex == 3) {
            delete store.getState().form.config.values;
        }
        this.setState({
    ...
    render() {
        ...
        const buildCmd = ()=> {
            return (<div>
                ...
                <div>{'\u00a0\u00a0\u00a0\u00a0'}-v /data:/data\</div>
                {this.state.masterNodes.length > 0 && (<div>{'\u00a0\u00a0\u00a0\u00a0'}--materNodes=
                    {this.state.masterNodes.map((entry, index)=> {
                        return <span>{index != 0 && ','}{entry.ip}</span>
                    })}\
                </div>)}
                {this.state.slaveNodes.length > 0 ? (<div>{'\u00a0\u00a0\u00a0\u00a0'}--slaveNodes=
                    {this.state.slaveNodes.map((entry, index)=> {
                        return <span>{index != 0 && ','}{entry.ip}</span>
                    })}\
                </div>) : (<div>{'\u00a0\u00a0\u00a0\u00a0'}--slaveNodes=
                    {this.state.masterNodes.map((entry, index)=> {
            ...
            switch (stepIndex) {
                ...
                case 3:
                    return (<div>
                        <h3>Platform Dashboard</h3>
                        <div>
                            <a href={'http://'+this.state.masterNodes[0].ip}>http://{this.state.masterNodes[0].ip}</a>
                        </div>
                    </div>);
                default:
                    return 'restart?';
